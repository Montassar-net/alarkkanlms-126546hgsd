...
document.getElementById('scheduleClassForm').addEventListener('submit', async (e) => {
    ...
    const token = localStorage.getItem('token');
    try {
        const response = await fetch('https://your-backend-api.com/classes', {
            method: 'POST',
            headers: { 
...
async function toggleClassStatus(classId, isOpen) {
    const token = localStorage.getItem('token');
    try {
        const response = await fetch(`https://your-backend-api.com/classes/${classId}/status`, {
            method: 'PUT',
            headers: { 
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
...
async function fetchEnrolledTrainees(classId) {
    const token = localStorage.getItem('token');
    try {
        const response = await fetch(`https://your-backend-api.com/classes/${classId}/enrollments`, {
            method: 'GET',
            headers: { 'Authorization': `Bearer ${token}` }
        });
        ...
        if (response.ok) {
            ...
            enrollments.forEach(enrollment => {
                const li = document.createElement('li');
                li.textContent = enrollment.traineeName;
